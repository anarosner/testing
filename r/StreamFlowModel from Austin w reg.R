...
if(run_option == 0){
	basin_area <- 10.81
	parameters <- c(0.934, 350.2514, 0.0000014, 0.62498, 0.4623, -0.7021, 9.9952, 202.6, 73.4, 28.3) #SCE
...
if (snow == 1){
	...
	#set initial parameter values
	stocflow <- ABCD(parameters,stoc.weather$TAVG,stoc.weather$PRCP,stoc.weather$ET,Sint,Gint,s_calib_num)
	histflow <- ABCD(parameters,hist.weather$TAVG,hist.weather$PRCP,hist.weather$ET,Sint,Gint,h_calib_num)
}
