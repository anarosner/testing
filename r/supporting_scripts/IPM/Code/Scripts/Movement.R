...
#Movement for Fish
if (move==T){
psiA <- array( NA,c(4,4,4,nIter) )
for ( season in 1:4 ){
  for (iter in 1:nIter){
  
   psiA[season,,,iter] <- t( pList$psiBeta[season,,,iter,1] )
    }
  }


psim <- array(0,c(4,4,4))
for ( season in 1:4 ){
  for (iter in 1:nIter){
    psim[season,,] <-  psim[season,,]+psiA[season,,,iter]
  }
}
for ( season in 1:4 ){
  psim[season,,] <-psim[season,,] / nIter
}
...
if (move==F) {
  psiA <- array( NA,c(4,nRiver,nRiver,nIter) )
  for ( season in 1:4 ){
    for (iter in 1:nIter){
    psiA[season,,,iter] <- diag(nRiver)
    }
  }  
}
