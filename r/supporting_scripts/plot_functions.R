...
#    par(mai=c(.25,.25,.01,.01)) #svg (inches)
   ...
   if (type=="precip") {
      ...
      plot(h.annual$year,h.annual$flow,xlim=c(min(h.annual$year),max(s.annual$year)),ylim=c(0,max(h.max$flow)),lwd=2,col="gray40",axes=F,xlab="",ylab="",pch=16)
      axis(side=2,at=p.h,col="white", las=2,cex.axis=p.sl,col.axis="slategray4") 
      axis(side=1,col="white", las=1,cex.axis=p.sl,col.axis="slategray4") 
      abline(h=p.h,col="grey",lty=3)

      points(s.annual$year,s.annual$flow,pch=18,lty=5,lwd=2,col="gray40")
      
      points(h.max$year,h.max$flow,lty=1,col="steelblue",pch=16)
      points(s.max$year,s.max$flow,lty=5,col="steelblue",pch=18)
      
      points(h.min$year,h.min$flow,lty=1,col="salmon",pch=16)
      points(s.min$year,s.min$flow,lty=5,col="salmon",pch=18)
