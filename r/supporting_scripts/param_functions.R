...
missing.param<-function() {
   ...
   for(i in 1:length(req)) {
      if (!(req[i] %in% names(param)))
         missing<-c(missing,req[i])
   }
   
   if (!is.null(config$preceding_step) ) {
      pre_config<-config$preceding_step
      if (is.null(param$preceding))
         missing<-c(missing,"pre_config")
      else {
         pre_param<-param$preceding
         for (i in 1:length(pre_config)) {
            if (!(pre_config[i] %in% names(pre_param)))
               missing<-c(missing,pre_config[i])
         }
      }
   }
...
# parse.preceding<-function() {
#    preceding<-parse.param()
