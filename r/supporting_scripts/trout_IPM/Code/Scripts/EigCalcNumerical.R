...
initial <- array(0,c(length(y)+1))
for (i in 1:(length(y)+1)){
  o <- ylong[i]
initial[i] <- init(o) 
}

initialN <- array(0,c(length(y)+1,nRiver))
for(river in 1:nRiver){
  initialN[,river] <- initial
}

initialN <- c(as.matrix(initialN))
...
for (trans in c(F,T)){
  ...
  for (season in 1:4){
    ...
    while(max(qmax)>tol) {
      ...
      for (river in 1:nRiver){
        riverlam[season,river] <- sum(Nt1[(river*(length(y)+1)-length(y)):(river*(length(y)+1))])/sum(Nt[(river*(length(y)+1)-length(y)):(river*(length(y)+1))])
      }
    ...
    if ( trans == F ) vectorsmetaKMyear[1,season,(river*(length(y)+1)-100):(river*(length(y)+1))]=riverArea[season,,]%*%(Nt[(river*(length(y)+1)-100):(river*(length(y)+1))]/sum(Nt)); 
    if ( trans == T ) vectorsmetaKMyear[2,season,(river*(length(y)+1)-100):(river*(length(y)+1))]=solve(riverArea[season,,])%*%(Nt[(river*(length(y)+1)-100):(river*(length(y)+1))]/sum(Nt));    
