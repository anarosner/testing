...
#year start, end, and count
y1<-h_daily[1,"YEAR"]
yn<-h_daily[nrow(h_daily),"YEAR"]
n_years<-yn-y1

#baseline shift
#calculate historic mean and var
#and slope of increase for shifted mean and var
...
s_daily$YEAR<-s_daily$YEAR+n_years+1 #start the year following the last year of the historic record
    # s_daily$YEAR<-seq(from=2010,length.out=n_years)
...
#calculate shifted mean and var for each year
...
#shift precip record w/ non-stationary mean and var
...
#calculate shifted mean for each year
s_daily$temp_shift<-change_temp_mean_y1+as.numeric(row.names(s_daily))*t_mean_slope
#shift temp records w/ non-stationary mean and var
...
###call plot functions 
h.clim<-h_monthly #(rename variables for plot functions)
s.clim<-s_monthly
