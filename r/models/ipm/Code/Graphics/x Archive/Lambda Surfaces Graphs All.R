...
metaLambdas2[1,4,,,] <- metaLambdas2[2,4,,,]
dimnames(metaLambdas) <- list(c("Main","Obear"),c("Westbrook","Jimmy","Mitchell","Obear"),c("Spring","Summer","Fall","Winter"),fvariables,tvariables)

metaLambdasdf <- melt(metaLambdas2)
names(metaLambdasdf) <- c("Population","Stream","Season","StandardizedFlow","StandardizedTemperature","Lambda")



testLambdas <- metaLambdas2[1,,,,]
metaLambdasdf <- melt(testLambdas)
...
c <- v  + facet_grid(Season~Stream) +  geom_tile(aes(fill=Lambda)) + scale_fill_gradient(low="black", high="white") + 
