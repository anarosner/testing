 ...
 metaLambdasdf <- melt(metaLambdas2^10)
 names(metaLambdasdf) <- c("Population","StandardizedFlow","StandardizedTemperature","Proportion")
 metaLambdasdf$Population <- factor(metaLambdasdf$Population,levels=c("Main","Obear"),ordered=T)
 #win.graph(); par(mfrow=c(1,1));
 v <- ggplot(metaLambdasdf, aes(StandardizedFlow, StandardizedTemperature, z=Proportion))
 c <- v  + facet_grid(.~Population) +  geom_tile(aes(fill=Proportion)) + scale_fill_gradient(low="black", high="white") +  
