...
#  vec <- vec/(2*range(vec)[2]);                          #This standardizes the values by the largest value
  if (pop==1){
    if(transpose==F) {vec=riverArea[season,,]%*%(Re(eigen(mat)$vectors[,1])); vec=vec/sum(vec)}; 
    if(transpose==T) {vec=solve(riverArea[season,,])%*%(Re(eigen(t(mat))$vectors[,1])); vec=vec/sum(vec) };   #Note that this is not rescaled
  }
