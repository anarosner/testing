...
if(means==T){
  for (river in 1:nRiver) {
    for (season in 1:4){
      for (i in 1:nIter){
        Sx[,,season,river] <- Sx[,,season,river]+ t(outer(y,y,sx,flow=flow,temp=temp,season=season,river=river,iter=i,params=pList)) 
      }
    }
  }
...
for (season in 1:4){
  for (river in 1:nRiver){
    ...
    if (season == 3) {
      demo[1,,,season,river] <- abind((abind(0,array(0,c(length(y),1)),along=1)),(abind(array(0,c(1,length(y))),Gxy[,,season,river],along=1)),along=2)
      demo[2,,,season,river] <- abind((abind(0,array(0,c(length(y),1)),along=1)),(abind(array(0,c(length(y))),Sx[,,season,river],along=1)),along=2)
      ...
      demo[6,,,season,river] <- abind((abind(0,array(0,c(length(y),1)),along=1)),(abind(array(SJx[1,1,season,river],c(length(y))),array(0,c(length(y),length(y))),along=1)),along=2)
    }
    if (season == 4) {
      demo[1,,,season,river] <- abind((abind(0,array(0,c(length(y),1)),along=1)),(abind(array(0,c(1,length(y))),Gxy[,,season,river],along=1)),along=2)
      demo[2,,,season,river] <- abind((abind(0,array(0,c(length(y),1)),along=1)),(abind(array(0,c(1,length(y))),Sx[,,season,river],along=1)),along=2)
...
for (rate in 1:6){
  for (season in 1:4){
    for (river in 1:nRiver){
     metaDemo[rate,season,((river*(length(y)+1)-length(y)):(river*(length(y)+1))),((river*(length(y)+1)-length(y)):(river*(length(y)+1)))] <- demo[rate,,,season,river]
    }
  }
}
