...
sx<-function(x,y,flow,temp,season,river,iter,params) {
...
gxy<-function(x,y,flow,temp,season,river,iter,params) { 
  type <- metaRiverA[river,2]
  
  stdX <- (x-params$lengthMean[season,type])/params$lengthSd[season,type]  
  g  <- ( (params$grBeta[1,season,type,iter]) +
    (params$grBeta[2,season,type,iter])* stdX +
    (params$grBeta[3,season,type,iter])*  flow*stdX+
    (params$grBeta[4,season,type,iter])* temp*stdX+
    (params$grBeta[5,season,type,iter])* (temp^2)*stdX+
    
    (params$grBeta[6,season,type,iter])* flow +
    (params$grBeta[7,season,type,iter])* flow*temp+
    (params$grBeta[8,season,type,iter])* flow*temp^2+
    
    (params$grBeta[9,season,type,iter])*  temp+
    (params$grBeta[10,season,type,iter])* temp^2+
    
    (params$grBeta[11,season,type,iter])*   flow*temp*stdX
  )
  
  
  sd <- (( mean(params$grSigmaBeta[season,type,,iter]) #+
