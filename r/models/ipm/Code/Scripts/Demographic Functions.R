...
#(out$pBeta[season,yr+1,type+1,iter,1])
...
gxy<-function(x,y,flow,temp,season,river,params) { 
 ...
 stdX <- (x-params$lengthMean[season,type])/params$lengthSd[season,type]  
  g  <- ( mean(params$grBeta[1,season,type,]) +
    ...
    mean(params$grBeta[3,season,type,])*  flow*stdX+
    mean(params$grBeta[4,season,type,])* temp*stdX+
    mean(params$grBeta[5,season,type,])* (temp^2)*stdX+
    
    mean(params$grBeta[6,season,type,])* flow +
     mean(params$grBeta[7,season,type,])* flow*temp+
     mean(params$grBeta[8,season,type,])* flow*temp^2+
     
     mean(params$grBeta[9,season,type,])*  temp+
     mean(params$grBeta[10,season,type,])* temp^2+
     
    mean(params$grBeta[11,season,type,])*   flow*temp*stdX
          )
