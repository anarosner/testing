...
getEigSens <- function(mat,transpose){
  ...
  if(transpose==F) {vec=Re(eigen(mat)$vectors[,1]);  }; 
  if(transpose==T) {vec=Re(eigen(t(mat))$vectors[,1]);  };   #Note that this is not rescaled
...
for (season in 1:4){
  ...
  KSensSeason[season,,] <- KSensSeason[season,,] * diag(1,nRiver,nRiver) %x% array(1,c((length(y)+1),(length(y)+1)))
  KElasSeason[season,,] <- KElasSeason[season,,] * diag(1,nRiver,nRiver) %x% array(1,c((length(y)+1),(length(y)+1)))
