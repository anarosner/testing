...
for (river in 1:nRiver) {
  for (season in 1:4){
    FEC[,,season,river] <- F1x[,,season,river]*F3x[,,season,river]
  }
}
...
for (season in 1:4){
  for (river in 1:nRiver){
    if (season == 2){
    K[,,season,river] <- abind((abind(0,F2xy[,,season,river],along=1)),(abind(FEC[,,season,river],Z[,,season,river],along=1)),along=2)
    }
    else{
    K[,,season,river] <- abind((abind(SJx[1,1,season,river],F2xy[,,season,river],along=1)),(abind(FEC[,,season,river],Z[,,season,river],along=1)),along=2)  
    }
    }
}
...
metaMc <- array(0,c(4,(length(y)+1)*nRiver,4))
zeros <- array(0,c(4,length(y)*nRiver))
for (season in 1:4){
