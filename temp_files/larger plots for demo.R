...
svg(filename="precip.svg",width=5,height=4)


par(mai=c(.5,.75,.4,.3)) #svg (inches)
#          par(mai=c(.75,.75,.1,.1)) #screen
    par(fg="white",col.main="black",cex=p.s,cex.main=p.s)
    par(family="serif")
...
par(cex.lab=.9)
    ...
    title(main="Mean Annual Precipitation",col="black", family="f3")
...
#write svg
svg(filename="temp.svg",width=5,height=4)
par(mai=c(.5,.75,.4,.3)) #svg (inches)
#          par(mai=c(.75,.75,.1,.1)) #screen
  par(fg="white",col.main="black",cex=p.s,cex.main=p.s)
  par(family="serif")
...
svg(filename="flow.svg",width=5,height=4)
    par(mai=c(.5,.75,.4,.3)) #svg (inches)
    #          par(mai=c(.75,.75,.1,.1)) #screen
    par(fg="white",col.main="black",cex=p.s,cex.main=p.s)
    par(family="serif")
